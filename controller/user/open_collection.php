...
$collection = isset($_GET['type']) ? trim($_GET['type']) : "";
$songs = [];
$collectionTitle = "";
$collectionLabel = "";
$collectionDescription = "";
$collectionGradient = "from-[#53e076]/20 via-[#1a1a1a] to-[#131313]";
$tableName = "";
$dateAlias = "";
...
if($collection === "liked")
{
    ...
    $collectionDescription = "All the songs you marked with the like button.";
    $collectionGradient = "from-[#4f1dff]/35 via-[#24104f] to-[#131313]";
    $tableName = "user_liked_songs";
    $dateAlias = "liked_at";
}
elseif($collection === "downloads")
{
    $collectionTitle = "Downloads";
    $collectionLabel = "Offline Collection";
    $collectionDescription = "All the songs you added to your downloads section.";
    $collectionGradient = "from-[#0f766e]/35 via-[#123b3a] to-[#131313]";
    $tableName = "user_downloads";
    $dateAlias = "downloaded_at";
}
else
{
...
"SELECT songs.*, songs.id AS song_id, $tableName.created_at AS $dateAlias
FROM $tableName
INNER JOIN songs
ON $tableName.song_id = songs.id
WHERE $tableName.user_id='$escapedCanonicalUserId'
...
<title><?php echo htmlspecialchars($collectionTitle); ?> - Melodix</title>
...
<div class="min-h-screen pb-[120px]">
    <section class="relative overflow-hidden border-b border-white/10">
        <div class="absolute inset-0 bg-gradient-to-b <?php echo $collectionGradient; ?>"></div>
        <div class="relative px-6 py-10 md:px-10 lg:px-12">
            <a href="library.php" class="inline-flex items-center gap-2 text-sm font-semibold text-white/80 hover:text-[#53e076]">
                <span class="material-symbols-outlined">arrow_back</span>
                Back to Library
            </a>
            <div class="mt-8 flex flex-col gap-8 md:flex-row md:items-end">
                <div class="flex h-64 w-64 items-center justify-center rounded-3xl shadow-2xl shadow-black/40 <?php echo $collection === 'liked' ? 'bg-gradient-to-br from-[#4f1dff] via-[#7340ff] to-[#c7f3db]' : 'bg-gradient-to-br from-[#0f766e] via-[#115e59] to-[#1f2937]'; ?>">
                    <span class="material-symbols-outlined text-[120px] text-white" style="font-variation-settings:'FILL' 1;">
                        <?php echo $collection === 'liked' ? 'favorite' : 'download_done'; ?>
                    </span>
                </div>
                <div class="max-w-3xl">
                    <p class="text-sm font-bold uppercase tracking-[0.35em] text-[#53e076]"><?php echo htmlspecialchars($collectionLabel); ?></p>
                    <h1 class="mt-4 text-5xl font-black leading-none md:text-7xl">
                        <?php echo htmlspecialchars($collectionTitle); ?>
                    </h1>
                    <p class="mt-5 max-w-2xl text-base text-white/70 md:text-lg">
                        <?php echo htmlspecialchars($collectionDescription); ?>
                    </p>
                    <div class="mt-6 flex flex-wrap items-center gap-4 text-sm text-white/60">
                        ...
                        <span>Available only to your account</span>
                    </div>
                    <div class="mt-8 flex flex-wrap items-center gap-4">
                        ...
                        class="inline-flex h-14 items-center gap-2 rounded-full bg-[#53e076] px-8 text-base font-bold text-black transition hover:scale-105 hover:bg-[#6af08a]">
                            ...
                            Play <?php echo htmlspecialchars($collectionTitle); ?>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-[28px] border border-white/8 bg-[#171717] p-3 md:p-5">
            ...
            <?php } else { ?>
                <div class="px-4 py-10 text-center text-white/55">
                    No songs found in this collection yet.
                </div>
            <?php } ?>
        </div>
    </section>
...
const songs = <?php echo json_encode($songs, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>;
